...
The site is deployed by [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml). Every push to `main` publishes `AC_Service_Field_Guide_Mobile_FINAL.html` as the site's `index.html`. The workflow can also be started manually from the repository's **Actions** tab.
